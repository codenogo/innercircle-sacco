...
export function Payouts() {
  ...
  const loadPayouts = useCallback(async (opts?: { append?: boolean; cursor?: string | null }) => {
    ...
    try {
      const path = cursor
        ? `/api/v1/payouts?size=${PAGE_SIZE}&cursor=${encodeURIComponent(cursor)}`
        : `/api/v1/payouts?size=${PAGE_SIZE}`
  ...
  }, [filter, payouts, search, memberMap])

  const completed = payouts.filter(p => p.status === 'COMPLETED')
  const pending = payouts.filter(p => p.status === 'PENDING' || p.status === 'PROCESSING' || p.status === 'APPROVED')
